...
double AstarSearch::estimateCost(const geometry_msgs::Pose & pose)
{
  ...
  // see: https://ompl.kavrakilab.org/workingWithStates.html
  // NOTE StateSpacePtr is a shared pointer to StateSpace. (generated by macro)
  ompl::base::ScopedState<ompl::base::SE2StateSpace> state(ompl_se2space_), goal_state(ompl_se2space_);

  state->setX(pose.position.x);
  state->setY(pose.position.y);
  state->setYaw(tf2::getYaw(pose.orientation));

  goal_state->setX(goal_pose_.position.x);
  goal_state->setY(goal_pose_.position.y);
  goal_state->setYaw(tf2::getYaw(goal_pose_.orientation));

  total_cost += ompl_rsspace_->distance(state.get(), goal_state.get()) * astar_param_.distance_heuristic_weight;
